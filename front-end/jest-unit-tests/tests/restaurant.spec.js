...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    // TESTE 1: Verifique se função `createMenu()` retorna qqum objeto que possui a chave `fetchMenu`, a qual tem como valor uma função.
    // ```
    ...
    // ```
    objetoRetornado = createMenu({ coxinha: 25, agua: 500 })
    objetoRetornado.order('coxinha');
    objetoRetornado.order('agua');
    objetoRetornado.order('coxinha');
    // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos
    expect(objetoRetornado.pay()).toEqual(605);
    // ```
